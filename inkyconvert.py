#!/usr/bin/env python3
# taken from https://github.com/RubenLagrouw/inkyconv
...
def convert(imgpath, outpath, example='InkypHAT-212x104.png', dimensions=(122, 122)):
    subprocess.run(
#        'convert {in_file} -auto-orient -geometry {x}x{y}^ -gravity center -crop {x}x{y}+0+0 -strip -type Palette -remap {ex} temp.png'
